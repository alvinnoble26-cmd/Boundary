...
public sealed class SliceAbilityTests
{
    ...
    public void UsesRequestedBalanceAndPresentationTiming()
    {
        ...
        Assert.That(SliceAbility.Radius, Is.EqualTo(10f));
        Assert.That(SliceAbility.ArcDegrees, Is.EqualTo(120f));
        Assert.That(SliceAbility.SwingDuration, Is.EqualTo(0.2f));
    ...
    {
        Vector3 edge = Quaternion.Euler(0f, 60f, 0f) * Vector3.forward * 10f;
        ...
        Assert.That(SliceAbility.IsInSlash(Vector3.zero, Vector3.forward, Vector3.forward * 10.01f), Is.False);
    }
    ...
    {
        Assert.That(SliceAirFracture.RemainingLifetime(1.25f), Is.EqualTo(2.1f).Within(0.001f));
        Assert.That(SliceAirFracture.RemainingLifetime(4f), Is.Zero);
        Assert.That(SliceAirFracture.RemainingLifetime(-1f), Is.EqualTo(3.35f).Within(0.001f));
    }
}
