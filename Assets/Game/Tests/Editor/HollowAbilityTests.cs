...
public sealed class HollowAbilityTests
{
    ...
    {
        Assert.IsTrue(HollowAbility.IsPointInsideBlast(
            new Vector3(0f, 0f, 69.9f), Vector3.zero, Vector3.forward));
    }
    ...
    {
        Assert.IsFalse(HollowAbility.IsPointInsideBlast(
            new Vector3(0f, 0f, 70.1f), Vector3.zero, Vector3.forward));
    }
...
{
    [Test]
    public void NormalMovementHasVisibleWindAndFasterMovementAmplifiesIt()
    {
        float idle = PlayerWindPresentation.CameraWindIntensity(0.5f, 1.2f, 24f);
        float normal = PlayerWindPresentation.CameraWindIntensity(7f, 1.2f, 24f);
        float fast = PlayerWindPresentation.CameraWindIntensity(18f, 1.2f, 24f);

        Assert.That(idle, Is.Zero);
        Assert.That(normal, Is.GreaterThanOrEqualTo(0.055f));
        Assert.That(normal, Is.LessThan(0.25f));
        Assert.That(fast, Is.GreaterThan(normal));
    }

    [Test]
    public void AbilitySpeedBaselineStaysBelowExplicitAbilityBoost()
    {
        float dash = PlayerWindPresentation.CameraWindIntensity(16f, 1.2f, 24f);
        float grapple = PlayerWindPresentation.CameraWindIntensity(
            new Vector3(3f, 15f, 2f).magnitude, 1.2f, 24f);

        Assert.That(dash, Is.LessThan(0.88f));
        Assert.That(grapple, Is.LessThan(0.88f));
    }
...
public sealed class ForceFieldWindPresentationTests
{
    ...
    [Test]
    public void AttractFlashUsesReducedRadiusAndPeakIntensity()
    {
        Assert.That(ForceField.AttractFlashRadius, Is.EqualTo(29.4f).Within(0.001f));
        Assert.That(ForceField.VisualBrightnessMultiplier, Is.EqualTo(9f).Within(0.001f));
        Assert.That(ForceField.AttractFlashPeakIntensity, Is.EqualTo(5670f).Within(0.001f));
        Assert.That(ForceField.AttractFlashDuration, Is.EqualTo(2.1f).Within(0.001f));
    }

    [Test]
    public void RepelFlashUsesReducedRadiusAndPeakIntensity()
    {
        Assert.That(ForceField.RepelFlashRadius, Is.EqualTo(29.4f).Within(0.001f));
        Assert.That(ForceField.RepelFlashPeakIntensity, Is.EqualTo(5670f).Within(0.001f));
