...
public sealed class PracticeModePanel : MonoBehaviour
{
    ...
    {
        if (GameManager.I == null || GameManager.I.State != GameManager.GameState.Menu) return;
    ...
    private void Build()
    {
        ...
        backdrop.color = new Color(0.005f, 0.012f, 0.035f, 0.99f);
        AddText(transform, "PRACTICE", 42, new Vector2(0f, 180f));
    ...
    private Button MakeButton(string label, float y)
    {
        ...
        obj.GetComponent<Image>().color = label == "Back"
            ? new Color(0.015f, 0.025f, 0.065f, 1f)
            : new Color(0.02f, 0.16f, 0.46f, 1f);
        Button button = obj.GetComponent<Button>();
        button.targetGraphic = obj.GetComponent<Image>();
        AddText(obj.transform, label, 28, new Vector2(0f, label == "Back" ? 0f : 12f));
        return button;
    }

    private static void AddText(Transform parent, string value, int size, Vector2 position)
    {
        Text text = new GameObject(value, typeof(RectTransform), typeof(Text)).GetComponent<Text>();
