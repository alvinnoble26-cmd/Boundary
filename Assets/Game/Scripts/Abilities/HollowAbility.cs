...
public sealed class HollowAbility : MonoBehaviour, IAbility
{
    ...
    private readonly List<Object> presentationResources = new List<Object>();
    ...
    private IEnumerator PlayPresentation(Vector3 direction, bool showCrispBlastEffect, float elapsedAtStart)
    {
        AcquireWorldGlow();
        ...
        Material purpleMaterial = TrackPresentationResource(CreateMaterial(Purple));
        Material energyMaterial = TrackPresentationResource(
            magicEnergyMaterial != null ? new Material(magicEnergyMaterial) : null);
        Material brightMaterial = TrackPresentationResource(CreateMaterial(BrightPurple));
        Material blackMaterial = TrackPresentationResource(CreateMaterial(VoidBlack));
        Material hotWhiteMaterial = TrackPresentationResource(CreateMaterial(new Color(5f, 4.2f, 5f, 1f)));
        Texture2D softTexture = TrackPresentationResource(CreateSoftParticleTexture());
        Material softPurpleMaterial = TrackPresentationResource(CreateParticleMaterial(softTexture));
        Material softWhiteMaterial = TrackPresentationResource(CreateParticleMaterial(softTexture));
        ...
        float blastStartedAt = Time.time - Mathf.Max(0f, elapsedAtStart - ChargeDuration);
    ...
    private T TrackPresentationResource<T>(T resource) where T : Object
    {
        if (resource != null)
            presentationResources.Add(resource);
        return resource;
    }

    private void CleanupPresentationResources()
    {
        for (int index = 0; index < presentationResources.Count; index++)
        {
            Object resource = presentationResources[index];
            if (resource != null)
                Destroy(resource);
        }
        presentationResources.Clear();
    }
