...
public class Cam : NetworkBehaviour
{
    ...
    private IEnumerator SetupCameraRoutine()
    {
        ...
        if (cam.TryGetComponent<UniversalAdditionalCameraData>(out UniversalAdditionalCameraData data))
        {
            ...
            // Slice's dimensional fracture samples the scene color to invert
            // and distort the air inside the cut. Force the opaque texture on
            // for each owning gameplay camera so the effect works on both
            // clients even when a quality-level URP asset overrides defaults.
    ...
    public void RequestBullseyeHitShake()
    {
        if (!isOwner || !SettingsMenu.ScreenShakeEnabled)
            return;

        StartScreenShake(0.2f, 0.07f);
    }

    public void RequestChargeHitShake()
    {
        if (!isOwner || !SettingsMenu.ScreenShakeEnabled)
            return;

        StartScreenShake(0.28f, 0.085f);
    }

    public void RequestSliceHitShake()
    {
        if (!isOwner || !SettingsMenu.ScreenShakeEnabled)
            return;

        StartScreenShake(0.24f, 0.075f);
    }
    ...
    {
        if (isOwner && isReady)
            GetFirstPersonArm()?.SetBullseyeKnifeActive(active, knifePrefab);
    }

    public void ThrowBullseyeKnife(Vector3 aimDirection, GameObject knifePrefab)
    {
        if (isOwner && isReady)
            GetFirstPersonArm()?.ThrowBullseyeKnife(aimDirection, knifePrefab);
    }

    public void SetChargeSwordActive(bool active, GameObject swordPrefab, GameObject lightningAuraPrefab)
    {
        if (isOwner && isReady)
            GetFirstPersonArm()?.SetChargeSwordActive(active, swordPrefab, lightningAuraPrefab);
    }

    public Vector3 GetChargeBallWorldPosition(Vector3 fallback)
    {
        if (!isOwner || !isReady)
            return fallback;
        FirstPersonArmPresentation presentation = GetFirstPersonArm();
        return presentation != null ? presentation.GetChargeBallWorldPosition() : fallback;
    }
    ...
    {
        if (isOwner && isReady)
            GetFirstPersonArm()?.SetSliceSwordActive(active, swordPrefab, airFractureEmitter);
    }

    public void SwingSliceSword(Vector3 aimDirection, GameObject swordPrefab)
    {
        if (isOwner && isReady)
            GetFirstPersonArm()?.SwingSliceSword(aimDirection, swordPrefab);
    }
