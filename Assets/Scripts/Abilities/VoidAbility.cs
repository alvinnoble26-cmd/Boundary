...
public sealed class VoidAbility : MonoBehaviour, IAbility
{
    ...
    public void BeginPresentation(Vector3 groundPosition, int seed, bool showEnemyHighlight)
    {
        ...
        presentation = StartCoroutine(PlayPresentation(groundPosition, seed, showEnemyHighlight));
    }

    private IEnumerator PlayPresentation(Vector3 groundPosition, int seed, bool showEnemyHighlight)
    {
        ...
        float nextSlashAt = startedAt + DarkTransitionSeconds;
    ...
    private static void RequestLocalFeedback()
    {
        ...
        if (mainCamera != null)
            mainCamera.GetComponentInParent<Cam>()?.RequestVoidShake();
